...
- Užívateľsky-viditeľný text píš v plynnej češtine. Bežne udomácnené výrazy (`protein`, `smoothie`, `pizza`, `burger`, `workout`, `meal prep`) sú OK — nepriekladaj ich nasilu kalkami.
- Žiadne anglické JSON field names v `_cs` poliach. (Field name môže byť `reason_cs`, ale jeho **hodnota** je vždy plynná čeština.)
- Vyhni sa celým anglickým vetám alebo frázam v naratíve (`"Add oats to hot water"`) — to je signál, že few-shot pretiekol.

### R2 — Alergie sú nedotknuteľné
...
### R5 — Žiadne halucinácie ingrediencií
