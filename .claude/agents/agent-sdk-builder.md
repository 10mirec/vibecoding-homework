...
Toto je **najkomplexnejší** subagent v projekte (jadro hodnoty NutriFlow). Superpowers metodológia je tu povinná, nie odporúčaná:

- **`brainstorming`** — pred implementáciou nového agenta alebo flow zmeny si s hlavným Claudeom prebrali: vstupný kontrakt, výstupný kontrakt, retry policy edge cases, fallback správanie, agent_runs metadata fields. Ak ti chýba čokoľvek z tohto, žiadaj brainstorm pred kódom.
- **`writing-plans`** — orchestrácia má veľa pohyblivých častí (5 agentov × happy/revise/fallback paths). Plán musí byť bite-sized: 1) base abstraction, 2) každý agent zvlášť so stub I/O, 3) wire orchestrator happy path, 4) revise loop, 5) fallback path, 6) agent_runs persistence. Nepáchaj všetko v jednom commite.
- **`test-driven-development`** — pre každý agent najprv test s mocked Agent SDK response:
...
- **`subagent-driven-development`** — implementácia každého agenta je samostatný task, kde dispatching parallel agents má zmysel (5 agentov → 5 paralelných subagentov so two-stage review). Pri zmene base abstraction toto nerob — base má dopad na všetkých.
- **`systematic-debugging`** — pri „agent vracia zlú schému“ nepláchaj prompt náhodne. Reprodukuj s identickým vstupom, izoluj (prompt? schema? SDK config?), formuluj hypotézu, testuj. Žiadne „skús to ešte raz“.
- **`verification-before-completion`** — `agent_runs` zapísané pre každý beh, end-to-end test prešiel, revise loop test prešiel, fallback test prešiel, manuálny smoke `curl` vrátil 200 s `WeeklyPlanResult`. „Pytest pre tento agent prešiel“ nie je hotové.
